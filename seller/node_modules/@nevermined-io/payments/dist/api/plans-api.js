...
export class PlansAPI extends BasePaymentsAPI {
     ...
     */
    getFiatPriceConfig(amount, receiver) {
     ...
     */
    async getPayAsYouGoPriceConfig(amount, receiver, tokenAddress = ZeroAddress) {
     ...
     */
    getCryptoPriceConfig(amount, receiver, tokenAddress) {
     ...
     */
    getERC20PriceConfig(amount, tokenAddress, receiver) {
     ...
     */
    getNativeTokenPriceConfig(amount, receiver) {
     ...
     */
    getExpirableDurationConfig(durationOfPlan) {
     ...
     */
    getFixedCreditsConfig(creditsGranted, creditsPerRequest = 1n) {
     ...
     */
    getDynamicCreditsConfig(creditsGranted, minCreditsPerRequest = 1n, maxCreditsPerRequest = 1n) {
     ...
     */
    setRedemptionType(creditsConfig, redemptionType) {
     ...
     * ```
     */
    async getAgentsAssociatedToAPlan(planId, pagination = new PaginationOptions()) {
        const query = API_URL_GET_PLAN_AGENTS.replace(':planId', planId) + '?' + pagination.asQueryParams();
        const url = new URL(query, this.environment.backend);
        const response = await fetch(url);
        if (!response.ok) {
            throw PaymentsError.fromBackend('Plan not found', await response.json());
     ...
     *
     */
    async getPlanBalance(planId, accountAddress) {
        ...
        const balanceUrl = API_URL_PLAN_BALANCE.replace(':planId', planId).replace(':holderAddress', holderAddress);
        const options = {
            method: 'GET',
            headers: {
     ...
     * ```
     */
    async mintPlanCredits(planId, creditsAmount, creditsReceiver) {
     ...
     * ```
     */
    async mintPlanExpirable(planId, creditsAmount, creditsReceiver, creditsDuration = 0n) {
