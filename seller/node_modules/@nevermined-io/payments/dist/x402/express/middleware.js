 ...
 */
function extractToken(req, headerNames) {
 ...
 */
function sendPaymentRequired(res, paymentRequired, message) {
...
export function paymentMiddleware(payments, routes, options = {}) {
    const { tokenHeader = DEFAULT_TOKEN_HEADERS, onPaymentError, onBeforeVerify, onAfterVerify, onAfterSettle, } = options;
    return (req, res, next) => {
        ...
        const handleRequest = async () => {
            ...
            if (!token) {
                ...
                sendPaymentRequired(res, paymentRequired, `Missing x402 payment token. Send token in ${X402_HEADERS.PAYMENT_SIGNATURE} header.`);
                return;
            }
            ...
            try {
                ...
                if (!verification.isValid) {
                    ...
                    sendPaymentRequired(res, paymentRequired, verification.invalidReason || 'Insufficient credits or invalid token');
                    return;
                }
                ...
                sendPaymentRequired(res, paymentRequired, error instanceof Error ? error.message : 'Payment verification failed');
            }
        };
