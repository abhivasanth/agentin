 ...
 */
async function sendRpcResult(res, result) {
...
async function bearerTokenMiddleware(handler, paymentsService, req, res, next) {
    ...
    const paymentExtension = agentCard.capabilities?.extensions?.find((ext) => ext.uri === 'urn:nevermined:payment');
...
export class PaymentsA2AServer {
    ...
    static start(options) {
        const { agentCard, executor, paymentsService, port, taskStore, basePath = '/', exposeAgentCard = true, exposeDefaultRoutes = true, expressApp, customRequestHandler, handlerOptions, hooks, } = options;
