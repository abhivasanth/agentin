...
export class PaymentsClient extends A2AClient {
     ...
     */
    async *sendA2AMessageStream(params) {
     ...
     */
    async *_parseA2AStream(response, originalRequestId) {
        if (!response.body) {
            ...
            while (true) {
                ...
                if (done) {
                    ...
                    if (eventDataBuffer.trim()) {
                        const result = this._processEventData(eventDataBuffer, originalRequestId);
                        yield result;
                    }
     ...
     */
    _processEventData(jsonData, originalRequestId) {
        if (!jsonData.trim()) {
        ...
        try {
            ...
            if (a2aStreamResponse.id !== originalRequestId) {
                ...
                throw new PaymentsError(`SSE Event's JSON-RPC response ID mismatch. Client request ID: ${originalRequestId}, event response ID: ${a2aStreamResponse.id}.`, 'payments_error');
            }
            if (this.isErrorResponse && this.isErrorResponse(a2aStreamResponse)) {
                const err = a2aStreamResponse.error;
                throw new PaymentsError(`SSE event contained an error: ${err.message} (Code: ${err.code}) Data: ${JSON.stringify(err.data)}`);
            }
            // Check if 'result' exists, as it's mandatory for successful JSON-RPC responses
            if (!('result' in a2aStreamResponse) || typeof a2aStreamResponse.result === 'undefined') {
                throw new PaymentsError(`SSE event JSON-RPC response is missing 'result' field. Data: ${jsonData}`, 'payments_error');
            }
            ...
            throw new PaymentsError(`Failed to parse SSE event data: "${jsonData.substring(0, 100)}...". Original error: ${e.message}`, 'payments_error');
        }
    }
    /**
     ...
     * @returns An AsyncGenerator yielding A2AStreamEventData (Message, Task, TaskStatusUpdateEvent, or TaskArtifactUpdateEvent).
     */
    async *resubscribeA2ATask(params) {
        const agentCard = await this.agentCardPromise;
        if (!agentCard.capabilities?.streaming) {
     ...
     * @returns The response from the agent.
     */
    async _postRpcRequestWithHeaders(method, params, headers) {
        const endpoint = await this._getServiceEndpoint();
        ...
        if (rpcResponse.id !== requestId) {
            throw new PaymentsError(`CRITICAL: RPC response ID mismatch for method ${method}. Expected ${requestId}, got ${rpcResponse.id}. This may lead to incorrect response handling.`, 'payments_error');
        }
        return rpcResponse;
    }
}
