...
import { createOAuthRouter, createCorsMiddleware, createJsonMiddleware, createHttpLoggingMiddleware, } from '../http/oauth-router.js';
...
let McpServerClass = null;
let ResourceTemplateClass = null;
/**
...
async function getMcpServerClass() {
    if (!McpServerClass) {
        try {
            ...
            throw new Error('Failed to load @modelcontextprotocol/sdk. Make sure it is installed: npm install @modelcontextprotocol/sdk');
        }
    }
...
export class McpServerManager {
     ...
     * Must be called before start().
     */
    registerResource(name, uriOrTemplate, config, handler, options = {}) {
        if (this.state !== ServerState.Idle) {
    ...
    async start(config) {
        ...
        try {
            ...
            // Mount OAuth router
            const environment = config.environment || this.payments.environmentName || 'staging_sandbox';
            ...
            if (this.expressApp && this.sessionManager) {
                mountMcpHandlers(this.expressApp, {
     ...
     */
    async registerHandlersWithPaywall() {
        ...
        for (const [name, registration] of this.tools) {
            ...
            const creditsOption = typeof options.credits === 'function'
                ? options.credits
                : typeof options.credits === 'number'
                    ? BigInt(options.credits)
                    : options.credits;
            const protectedHandler = this.paywallDecorator.protect(async (args, extra, paywallContext) => {
        ...
        for (const [uriOrTemplate, registration] of this.resources) {
            ...
            const creditsOption = typeof options.credits === 'function'
                ...
                : typeof options.credits === 'number'
                    ? BigInt(options.credits)
                    : options.credits;
            const protectedHandler = this.paywallDecorator.protect(async (uri, extra, paywallContext) => {
        ...
        for (const [name, registration] of this.prompts) {
            ...
            // Register with MCP server
            const sdkPromptConfig = {
                title: promptConfig.title,
                description: promptConfig.description,
            };
            if (promptConfig.argsSchema) {
                const schema = promptConfig.argsSchema;
 ...
 */
export function createServerManager(payments) {
