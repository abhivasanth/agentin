...
import { buildProtectedResourceMetadata, buildMcpProtectedResourceMetadata, buildAuthorizationServerMetadata, buildOidcConfiguration, buildServerInfoResponse, } from './oauth-metadata.js';
import { isClientRegistrationRequest, processClientRegistration, ClientRegistrationError, } from './client-registration.js';
/**
...
export function createOAuthRouter(options) {
    ...
    const { baseUrl, agentId, environment, serverName = 'mcp-server', tools = [], resources = [], prompts = [], scopes, oauthUrls, protocolVersion, enableOAuthDiscovery = true, enableClientRegistration = true, enableHealthCheck = true, enableServerInfo = true, version = '1.0.0', description, onLog, } = options;
...
export function createCorsMiddleware(origins = '*') {
    return function corsMiddleware(req, res, next) {
        ...
        res.setHeader('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept, Authorization, mcp-session-id, mcp-protocol-version');
 ...
 * ```
 */
export function createHttpLoggingMiddleware(onLog) {
    const log = onLog ||
        ((_message) => {
            // Intentionally empty - no logging when onLog not provided
        });
    return function httpLoggingMiddleware(req, res, next) {
        ...
        // Build log message
        const logParts = [`[${timestamp}]`, `${method}`, url, `IP: ${ip}`];
        if (authHeader) {
