 ...
 */
export function isClientRegistrationRequest(body) {
    if (!body || typeof body !== 'object')
        return false;
    const obj = body;
    return !!(obj.redirect_uris ||
        obj.grant_types ||
 ...
 */
export function validateClientRegistrationRequest(request) {
    // redirect_uris is required and must be a non-empty array
    if (!request.redirect_uris ||
        !Array.isArray(request.redirect_uris) ||
        request.redirect_uris.length === 0) {
        throw new ClientRegistrationError('invalid_request', 'redirect_uris is required and must be a non-empty array');
    }
    ...
    if (request.grant_types) {
        for (const grantType of request.grant_types) {
            if (!validGrantTypes.includes(grantType)) {
                throw new ClientRegistrationError('invalid_client_metadata', `Unsupported grant_type: ${grantType}`);
            }
        }
    }
    ...
    if (request.token_endpoint_auth_method &&
        ...
        throw new ClientRegistrationError('invalid_client_metadata', `Unsupported token_endpoint_auth_method: ${request.token_endpoint_auth_method}`);
    }
}
/**
