 ...
 */
function buildOAuthUrls(frontendUrl, backendUrl) {
 ...
 * @returns OAuth URLs configuration
 */
function getOAuthUrlsForEnvironment(environment) {
 ...
 */
export function getOAuthUrls(environment, overrides) {
 ...
 * ```
 */
export function buildOidcConfiguration(config) {
    const oauthUrls = getOAuthUrls(config.environment, config.oauthUrls);
    const scopes = config.scopes || [...DEFAULT_SCOPES];
 ...
 */
export function buildServerInfoResponse(config, options) {
    const oauthUrls = getOAuthUrls(config.environment, config.oauthUrls);
    const scopes = config.scopes || [...DEFAULT_SCOPES];
    return {
        name: config.serverName || 'MCP Server',
        version: options?.version || '1.0.0',
        description: options?.description || 'MCP server with Nevermined OAuth integration via Streamable HTTP',
        endpoints: {
