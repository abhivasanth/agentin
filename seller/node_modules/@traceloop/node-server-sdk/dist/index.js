...
class Telemetry {
    ...
    constructor() {
        ...
        if (this.telemetryEnabled) {
            this.posthog = new posthogNode.PostHog("phc_JMTeAfG8OpaPsyHzSBtqquMvko1fmOHcW0gyqLCrF3t");
        }
    }
    getAnonId() {
        if (this.anonId) {
        ...
        try {
            if (!fs__namespace.existsSync(Telemetry.ANON_ID_PATH)) {
                fs__namespace.mkdirSync(path__namespace.dirname(Telemetry.ANON_ID_PATH), { recursive: true });
                const anonIdFile = fs__namespace.openSync(Telemetry.ANON_ID_PATH, "w");
                this.anonId = uuid.v4();
                fs__namespace.writeSync(anonIdFile, this.anonId);
                fs__namespace.closeSync(anonIdFile);
            }
            else {
                const anonIdFile = fs__namespace.openSync(Telemetry.ANON_ID_PATH, "r");
                this.anonId = fs__namespace.readFileSync(anonIdFile, "utf8");
                fs__namespace.closeSync(anonIdFile);
            }
            return this.anonId;
        }
    ...
    capture(event, properties) {
        if (this.telemetryEnabled && this.posthog) {
            this.posthog.capture({
                ...
                event,
                properties: Object.assign(Object.assign({}, properties), this.getContext()),
            });
            this.posthog.flush();
        }
    }
...
Telemetry.ANON_ID_PATH = `${os__namespace.homedir()}/.cache/traceloop/telemetry_anon_id`;
...
const initialize = (options = {}) => {
    ...
    if (options.traceloopSyncEnabled === undefined) {
        if (process.env.TRACELOOP_SYNC_ENABLED !== undefined) {
            options.traceloopSyncEnabled = ["1", "true"].includes(process.env.TRACELOOP_SYNC_ENABLED.toLowerCase());
        }
        else {
...
const transformAiSdkSpanName = (span) => {
...
const transformPromptMessages = (attributes) => {
    if (AI_PROMPT_MESSAGES in attributes) {
        try {
            ...
            messages.forEach((msg, index) => {
                ...
                attributes[`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.${index}.role`] = msg.role;
            });
...
const transformCompletionTokens = (attributes) => {
    if (AI_USAGE_COMPLETION_TOKENS in attributes) {
        attributes[`${aiSemanticConventions.SpanAttributes.LLM_USAGE_COMPLETION_TOKENS}`] =
            attributes[AI_USAGE_COMPLETION_TOKENS];
        delete attributes[AI_USAGE_COMPLETION_TOKENS];
    }
};
const calculateTotalTokens = (attributes) => {
    const promptTokens = attributes[`${aiSemanticConventions.SpanAttributes.LLM_USAGE_PROMPT_TOKENS}`];
    const completionTokens = attributes[`${aiSemanticConventions.SpanAttributes.LLM_USAGE_COMPLETION_TOKENS}`];
...
const transformVendor = (attributes) => {
    if (AI_MODEL_PROVIDER in attributes) {
        ...
        if (vendor && vendor.startsWith("openai")) {
            attributes[aiSemanticConventions.SpanAttributes.LLM_SYSTEM] = "OpenAI";
        }
        else {
            attributes[aiSemanticConventions.SpanAttributes.LLM_SYSTEM] = vendor;
        }
        delete attributes[AI_MODEL_PROVIDER];
    }
};
const transformAiSdkAttributes = (attributes) => {
...
const initInstrumentations = () => {
    ...
    anthropicInstrumentation = new instrumentationAnthropic.AnthropicInstrumentation({ exceptionLogger });
...
const manuallyInitInstrumentations = (instrumentModules) => {
    const exceptionLogger = (e) => Telemetry.getInstance().logException(e);
    const enrichTokens = (process.env.TRACELOOP_ENRICH_TOKENS || "true").toLowerCase() === "true";
    ...
    if (instrumentModules === null || instrumentModules === void 0 ? void 0 : instrumentModules.langchain) {
...
const startTracing = (options) => {
    ...
    _sdk = new sdkNode.NodeSDK({
        resource: resources.resourceFromAttributes({
            [semanticConventions.ATTR_SERVICE_NAME]: options.appName || process.env.npm_package_name,
        }),
        spanProcessors,
...
function withEntity(type, { name, version, associationProperties, traceContent: overrideTraceContent, inputParameters, suppressTracing: shouldSuppressTracing, }, fn, thisArg, ...args) {
...
class LLMSpan {
    ...
    reportRequest({ model, messages, }) {
        ...
        messages.forEach((message, index) => {
            this.span.setAttributes({
                [`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.${index}.role`]: message.role,
                [`${aiSemanticConventions.SpanAttributes.LLM_PROMPTS}.${index}.content`]: typeof message.content === "string"
                    ? message.content
    ...
    reportResponse({ model, usage, completions, }) {
        this.span.setAttribute(aiSemanticConventions.SpanAttributes.LLM_RESPONSE_MODEL, model);
        if (usage) {
            this.span.setAttributes({
                [aiSemanticConventions.SpanAttributes.LLM_USAGE_PROMPT_TOKENS]: usage.prompt_tokens,
                [aiSemanticConventions.SpanAttributes.LLM_USAGE_COMPLETION_TOKENS]: usage.completion_tokens,
        ...
        completions === null || completions === void 0 ? void 0 : completions.forEach((completion, index) => {
            this.span.setAttributes({
                [`${aiSemanticConventions.SpanAttributes.LLM_COMPLETIONS}.${index}.finish_reason`]: completion.finish_reason,
                [`${aiSemanticConventions.SpanAttributes.LLM_COMPLETIONS}.${index}.role`]: completion.message.role,
                [`${aiSemanticConventions.SpanAttributes.LLM_COMPLETIONS}.${index}.content`]: completion.message.content || "",
            });
        });
    }
}
...
function withLLMCall({ vendor, type }, fn, thisArg) {
    const span = getTracer().startSpan(`${vendor}.${type}`, {}, api.context.active());
